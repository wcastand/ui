...
const useStore = create<State>((set, get) => {
	let color = ""
	for (let i = 0; i < 6; i++)
		color += choices[randomInteger(0, choices.length - 1)]
	return {
		color,
		last: null,
		current: ["", "", "", "", "", ""],
	...
	return (
		<div className={tw(container)} id="colormind">
			<div className={tw(section)}>
				<div
					className={tw(
						"flex flex-col md:(flex-row) gap-8 justify-center items-center",
					)}
				>
					<div className={tw("flex-1 flex flex-row md:(flex-col) gap-4")}>
						<div
							className={tw("w-24 h-24 rounded border border-gray-300", color)}
						/>
						<div
							className={tw(
						...
						<div className={tw("flex flex-row gap-4")}>
							<div className={tw("flex flex-col")}>
								{store.guessed.map((c, idx) => (
									<span
										key={`guessed_${idx}`}
										className={tw(cell, c ? "text-green-400" : "text-gray-800")}
									>
										{c ? store.color[idx] : "?"}
									</span>
								))}
				...
				<div
					className={tw("absolute bottom-2 left-2 text-sm flex flex-col px-4")}
				>
					<span>
						Colormind, like mastermind but if you are good with color, you can
						win easy (it's hard).
					</span>
					<span>
