...
const useStore = create<State>((set, get) => ({
	...
	draw: (ctx) => {
		...
		if (get().focused && document.visibilityState === 'visible') {
			...
			switch (direction) {
				case 'N':
					path.unshift({ x: pos.x, y: pos.y - BLOCK_SIZE < 0 ? 100 - BLOCK_SIZE : pos.y - BLOCK_SIZE })
					break
				case 'S':
					path.unshift({ x: pos.x, y: pos.y + BLOCK_SIZE >= 100 ? 0 : pos.y + BLOCK_SIZE })
					break
				case 'W':
					path.unshift({ x: pos.x - BLOCK_SIZE < 0 ? 100 - BLOCK_SIZE : pos.x - BLOCK_SIZE, y: pos.y })
					break
				case 'E':
					path.unshift({ x: pos.x + BLOCK_SIZE >= 100 ? 0 : pos.x + BLOCK_SIZE, y: pos.y })
					break
			}
			pos = path[0]
			...
			// test eating food
			if (pos.x === food.x && pos.y === food.y) set((s) => ({ length: s.length + 1, food: getPosition() }))
		...
		ctx.fillStyle = 'white'
		for (let i = 0; i < get().length; i++) ctx.fillRect(path[i].x, path[i].y, BLOCK_SIZE, BLOCK_SIZE)
	},
}))
...
function Snake({ focused }: { focused: boolean }) {
	...
	return (
		...
		>
			<div className={tw(title, 'bg-gradient-to-b from-gray-100 to-gray-300 text-gray-800 flex-1 w-full font-bold text-center text-lg')}>
				Snake
			</div>
			<div
				className={tw(
			...
			>
				<span className={tw(title, 'flex-1 pl-1 text-left')}>Score:{state.length}</span>
			</div>
			...
			<div className={tw('relative w-full h-8 bg-gradient-to-b from-gray-100 to-gray-400')}>
